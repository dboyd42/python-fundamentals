...
NAME = 0    # cost[] index/position
AMOUNT = 1  # cost[] index/position
...
# Function get_costs
# Description: Gets the montly costs from the user,
#              sums monthly costs
# Calls: None
# Parameters: costs []
# Returns: None
...
def get_costs(costs):
    ...
    for i in range(len(costs)):
        if (i < len(costs)-1):
            # Get costs from user
            costs[i][AMOUNT] = float(input('Enter the amount for the ' \
                + costs[i][NAME] + ': $'))
            # Accumulate costs
...
# Description: Displays the montly costs and total
# Calls: None
# Parameters: costs[]
# Returns: None
...
def display_costs(costs):
    ...
    # Declare variables
    width = 9  # string width

    print('Monthly Charges')
    print('===============')
    for i in range(len(costs)):
        if (len(costs[i][NAME]) < width):
            print(costs[i][NAME].upper(), '\t\t: $', format(costs[i][AMOUNT], \
                  ',.2f'), sep='')
        else:
            print(costs[i][NAME].upper(), '\t: $', format(costs[i][AMOUNT], \
                  ',.2f'), sep='')
